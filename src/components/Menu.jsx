...
const Menu = ({
  ...
  const allCategories = [
    AllProductTxt,
    ...new Set(categorias.map((category) => category.nombre)),
  ];
  ...
  const filterCategory = (category) => {
    ...
    if (category === AllProductTxt) {
      ...
      filteredData = allProd.filter(
        (productos) => productos.categorias.nombre === category
      );
    }
  ...
  return (
    <div className="cardtwo-container cardtwo-margin">
      <Row
        ...
        xl={4}
        className="g-4"
        style={{ margin: "3rem", paddingTop: "20px" }}
      >
        <div className="w-100">
          <h1>Productos</h1>
          <hr />
          ...
          <div key={menu._id} className="container-card-food">
            <Card className="card-food border-light secondary" bg="dark">
              ...
              <Card.Body className="card-body">
                ...
                <Card.Text className="price-food">$ {menu.precio}</Card.Text>
              </Card.Body>
              <Card.Footer>
                {menu.categorias.estado === "activo" ? (
                  <Button variant="light" onClick={() => agregarProducto(menu)}>
                    Añadir al carrito
                  </Button>
                ) : (
